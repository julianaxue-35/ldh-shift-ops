...
(function (root, factory) {
  ...
  function caseList(tasks, now) {
    ...
    const open = sortQueue(tasks.filter(t => !t.done), now);
    const completed = tasks.filter(t => t.done && now - doneAt(t) <= 24 * HOUR).sort((a, b) => doneAt(b) - doneAt(a));
  ...
  return { RED_FLAGS, EMERGENCY_SIGNS, signLabel, OFFSITE_NAMES, CHLORSIG_OPTIONS, stageOf, caseList, medCopyText, TIER_LABELS, TIER_TARGETS, LOCATIONS, tierFromFlags, normTier, statusOf, sortQueue,
