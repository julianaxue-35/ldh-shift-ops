...
(async () => {
  ...
  try {
    ...
    const tableText = await page.textContent('#queue-tbody');
    t.ok(tableText.indexOf('C-URG') < tableText.indexOf('E-R50'), 'highest tier first: an amber Urgent case is listed above a 50-hour Routine case');
    t.ok(tableText.indexOf('B-RED') < tableText.indexOf('C-URG'), 'Red flag cases come before Urgent ones');
